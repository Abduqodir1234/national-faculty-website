...
describe("Majors Delete API tests",()=>{
    ...
    it("check delete api working well or not",()=>{
        cy.deleteRequest(`/resource/category/${id}/uz`,token)
        .then(res=>{
    ...
    it("send request without token",()=>{
        cy.deleteRequest(`/resource/category/${id}/uz`)
        .then(res=>{
    ...
    it("send request with unexist id",()=>{
        cy.deleteRequest(`/resource/category/${123456789123}/uz`,token)
        .then(res=>{
    ...
    it("send request without lang",()=>{
        cy.deleteRequest(`/resource/category/${id}`)
        .then(res=>{
            expect(res.status).to.eq(404)
            expect(res.body.error).to.eq(true)
    ...
    it("send request with ru lang",()=>{
        cy.deleteRequest(`/resource/category/${id}/ru`)
        .then(res=>{
            expect(res.status).to.eq(403)
            expect(res.body.error).to.eq(true)
            expect(res.body.message).to.eq('No token')
    ...
    it("send request with en lang",()=>{
        cy.deleteRequest(`/resource/category/${id}/en`)
        .then(res=>{
            expect(res.status).to.eq(403)
            expect(res.body.error).to.eq(true)
            expect(res.body.message).to.eq("No token")
    ...
    it("send request with unsupported lang",()=>{
        cy.deleteRequest(`resource/category/${id}/de`)
        .then(res=>{
